...
\textbf{Inverse limits}
  ...
  \item
        Let $M$ be a fintely-generated module. Then there exists a free module $F$ and a (potentially infinitly-generated) module of relations $N$ such that $M = F / N$. Let $I(M)$ be the set of fintely-generated ideals of submodule of relation $N$ of $M$ ordered by inclusion. By the previous problem, there is a directed family of finitely-generated modules $N_A = A$ and a directed family of morphisms $f_{AB}$ indexed by $I(M)$ admitting $N$ as a direct limit. In other words, $M = F/\lim_A A$ where the limit is taken over all finitely-generated submodules of the relation module $N$.

        To proceed, we prove that $F/\lim_A A = \lim_A F/A$. In fact, consider the exact sequence
        $$0 \to A \hookrightarrow F \fto{\mod A} F/A \to 0.$$
        Taking, $g_{AB} = \id_F$ and $h_{AB} = x + A \mapsto x + B$ in the proof of exercise (21), we get the exact sequence

        $$0 \to \lim_A A \hookrightarrow F \to \lim_A F/A \to 0,$$
        which proves the assertion $M = F/\lim_AA = \lim_A F/A$. Since $F/A$ is fintely-presented module, the statement is proved for $M$ finitely-generated.

        If $M$ is not finitely-generated, then by problem (24),
        $$M = \lim_B B,$$
        where the limit is taken over all fintely-generated submodules of $M$. We now define the index set
        $$J = \underset{B \text{ fintely-generated}}{ \bigcup_{B \subset M} } \{(B, A) : A \in I(B)\}$$

        using the ordering $(A, B) \leq (C, D)$ if and only if $A \subseteq C$ or $A = B$ and $B \leq D$. The directed family of morphisms $f_{A'B'}: A' \to B'$ indexed by $I(M)$ associated with the direct limit $M$, and the directed family of morphisms between finitely-presented modules $g^{C}_{AB}: A \to B$ for a fixed finitely-generated module $C$ indexed by $I(C)$ naturally induce a directed family of finitely-presented submodules of the free module $F$ directed by the index set $J$, with direct limit $M$.

  \item
        Let $(f_i) \in \lim \Hom(E, M_i)$ with the direct family of morphisms induced by $f_{ij}: M_i \to M_j$. Since $f_j = f_{ij} \circ f_i$, we can define (naturally) a map $\phi: \lim_{i} \Hom(E, M_i) \to \Hom(E, \lim M_i)$ defined by $$\phi((f_i)) = e \mapsto (f_i(e)).$$

        Now let $(f_i) \in \ker \phi$, i.e., $(f_i(e)) = [0]$ for all $e \in E$. If $E$ is finitely generated, say by $(x_1, \dots, x_m)$, then there is a $N \in I$ such that $f_i(x_j) = 0$ for all $i \geq N$ and all $j$.
        This implies $f_i = 0$ for all $i \geq N$ which implies $(f_i) = [0]$.

        For the rest of the statement, first let $E = R^n$ for a ring $R$. Then we have $\Hom(R^n, \lim M_i) = \Hom(R, \lim M_i)^n$ and $\lim \Hom(R^n, M_i) = \lim \Hom(R, M_i)^n = (\lim \Hom(R, M_i))^n$. Thus, it suffices to show that $\phi$ is isomorphsim for $E = R$.

        Let $g \in \Hom(R, \lim M_i)$ such that $g(1) = (e_i) \in \lim M_i$.  Defining $f_i(1) = e_i$, and  $ f = (f_i) \in \lim \Hom(R, M_i)$, we see that $\phi$ is surjective, and thus isomorphism.

        Finally, when $E$ is finitely presented, let $\psi_k: \lim \Hom(F_k, M_i)$. Applying the snake lemma, we have the exact sequence
        $$0 = \ker \psi_1 \to \Coker \phi \to \Coker \psi_0 = 0.$$
        Hence $\Coker \phi = 0$, making it isomorphism for this case.
  ...
  \item
        ...
        Let $x \in A_i$ and $y \in A_j$ for some $i, j$. Let $(\cdot)$ be the binear operator associated with the $k-$algebra $A$. Define the natural product $(*)$
        $$(x + A_{i-1}) * (y + A_{j-1}) =  (x \cdot y + A_{i+j-1})$$
        By this product, $\gr_i(A)\gr_j(A) \subseteq \gr_{i+j}(A).$

  \item
        \begin{enumerate}[(a)]
          \item
                Define
                $$\gr_i(L)(x + A_{i-1}) = L(x) + B_{i-1}.$$

                Since $L(A_i) \subseteq B_i$, $\gr_i(L)(A_i) \subseteq B_i/B_{i-1} = \gr_i(B)$. Similarly,
            \begin{align*}
              \gr_i(L)\big((c + A_{-1}) * (a + A_{i-1})\big)
                &= \gr_i(L)\big(ca + A_{i-1}\big) \\
                &= L(ca) + B_{i-1} \\
                &= L(c)\,L(a) + B_{i-1} \\
                &= (L(c) + B_{-1}) * (L(a) + B_{i-1}) \\
                &= \gr_0(L)(c + A_{-1}) * \gr_i(L)(a + A_{i-1}) \,.
            \end{align*}

          \item
                \begin{enumerate}
                  \item[$\hookrightarrow$] Let $x \in A$ such that $L(x) = 0$. Let $i$ be the smallest $i$ such that $x \in A_i$. By definition, $\gr_i(L)(x + A_{i-1}) = L(x) + B_{i-1} = 0 + B_{i-1} \implies x \in A_{i-1}$ a contradiction unless $x \in A_{-1} = \set{0}$.

                  \item[$\twoheadrightarrow$] Let $x \in B$. Then there is smallest $i$ such that $x \in B_i$ and consequently, $x + B_{i-1} \in \gr_i{B}.$ By surjectivness of $\gr_i(L)$, there is $y_i$ such that $x - L(y_i) \in B_{i-1}$. Applying this procedure $i$ more times, we get $x - \sum_{j = 0}^iL(y_j) \in B_{-1} = \{0\}$. This implies $x = \sum_{j = 0}^iL(y_j) = L\big(\sum_{j = 0}^iy_j\big)$ showing $L$ is surjective.
                \end{enumerate}
        \end{enumerate}
  ...
  \item
        ...
        \begin{enumerate}[(a)]
          \item The given definition of $\nn_c$ is equivalent to
                $$\nn_c := \set{X = [x_{ij}] \in \nn : x_{ij}  = 0 \text{ if } j - i < n - c} \quad \text{ for  } 0 \leq c \leq n - 1.$$

                By the trivial action of component-wise addition and scalar multiplicaiton $\nn_c$ is a vector space over the field $k$. What remains is to show the standard matrix multiplication is closed under $\nn_c$ as it is bi-linear by definition. To see that consider the multiplication of two matrices $X = [x_{ij}], Y = [y_{ij}] \in \nn_c$, $XY$ is given by the $[\sum_{k} x_{ik}y_{kj}]$. For an entry, $\sum_{k} x_{ik}y_{kj} \neq 0$, we require, $k - i \geq n - c$ and $j - k \geq n - c$ or in other words, $j - c \leq n - (2c - n)$. Hence $XY \in \nn_{2c - n} \subseteq \nn_{c}$. Since $2c - n < c$ for all $c$, $X^{c+1} = 0$.

          \item
                Associativity follows from associativity of standard matrix mutliplication. Since $0 \in \nn$, $I \in U$. Finally, for every $I + X$, by nilpotence $I + Y = \prod_{k\geq 0} (I - X^{2k})$ is a unique inverse of $I + X$ hence $U$ satisfies all the group properties.

          \item
                Let $\exp(X) = I + X + \frac{1}{2!} X^2 + \cdots$ and $\log(I + Y) = Y - \frac{1}2Y^2 + \frac{1}{3}Y^3 - \cdots$. Injectivity of $\exp$ directly follows from the definition.

                Let $Z = \exp(\log(I + Y))$, then we have
                $$Z =  I + \sum_{n = 1}^{\infty} \dfrac{1}{n!} Y^n \sum_{i_1 + \cdots + i_n = k} \dfrac{(-1)^{k + n}}{i_1 \cdots i_n}.$$

                The RHS famously evaluates to $I + Y$. Hence, $\log$ is a well-defined inverse of $\exp$ making it a bijection.
        \end{enumerate}
